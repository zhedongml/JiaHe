...
Result CameraModel::SetMLExposureAuto()
{
	if (m_bConnected)
	{
		MLResult mlret = m_pCamera->SetMLExposureAuto();
		if (mlret.code != 1)
		{
			return Result(false, mlret.msg);
		}
		return Result();
	}
	else
	{
		return Result(false, "MV is not connected.");
	}
}

Result CameraModel::SetExposureTime(double time)
{
	if (m_bConnected)
	{
		MLResult mlret = m_pCamera->SetExposureTime(time);
		if (mlret.code != 1)
		{
			return Result(false, mlret.msg);
		}
...
Result CameraModel::StopGrabbing()
{
	if (m_bConnected)
	{
		MLResult mlret = m_pCamera->StopGrabbing();
		if (mlret.code != 1)
		{
			return Result(false, mlret.msg);
		}
...
Result CameraModel::StartGrabbing()
{
	if (m_bConnected)
	{
		MLResult mlret = m_pCamera->StartGrabbingAsync();
		if (mlret.code != 1)
		{
			return Result(false, mlret.msg);
		}
...
Result CameraModel::GrabOne()
{
	if (m_bConnected)
	{
		MLResult mlret = m_pCamera->GrabOne();
		if (mlret.code != 1)
		{
			return Result(false, mlret.msg);
		}
